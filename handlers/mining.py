...
async def mining_menu(message: types.Message):
    ...
    if timers[3]:  # upgrade_end
        ...
        if datetime.now() < end_time:
            ...
            await message.answer(f"🏗 **БУДІВНИЦТВО!**\nЗалишилось: {rem} хв.")
            return
        else:
    ...
    if mine_lvl == 0:
        builder.button(text=f"🏗 Будувати (💰{price})", callback_data=f"build:{price}")
    else:
        if a1 > 0 or a2 > 0:
            builder.button(text=f"📥 Зібрати (+{a1}/+{a2})",
                           callback_data=f"col:{a1}:{res[0]['col']}:{a2}:{res[1]['col']}")
        builder.button(text=f"🏭 Розширити ({mine_lvl + 1}-й) 💰{price}", callback_data=f"build:{price}")
    builder.adjust(1)

    stats = (f"🏭 **{planet}**\nЗаводів: {mine_lvl}\nНакопичено: {res[0]['emoji']} {a1} / {res[1]['emoji']} {a2}")
...
async def build(cb: types.CallbackQuery):
    ...
    if db.get_family_resources(fid)[0] >= price:
        ...
        await cb.message.edit_text(f"🏗 Роботи почались ({BUILD_TIME} хв).")
    else:
...
async def collect(cb: types.CallbackQuery):
    ...
    await cb.message.edit_text("✅ Зібрано!")